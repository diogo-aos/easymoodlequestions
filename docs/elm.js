...
var $author$project$Main$init = function (_v0) {
	var initialModel = {conversions: _List_Nil, files: _List_Nil, log: _List_Nil, sep: '\t'};
...
var $author$project$Main$Conversion = F4(
	function (file, questions, errors, result) {
		return {errors: errors, file: file, questions: questions, result: result};
	});
var $author$project$Main$multipleChoiceAnswerTemplate = F2(
	function (text, correct) {
		return '\n     <answer fraction=\"' + ((correct ? '100' : '0') + ('">\n       <text>' + (text + ('</text>\n       <feedback><text>' + ((correct ? 'Correcto!' : 'Incorrecto.') + '</text></feedback>\n     </answer>\n')))));
	});
var $author$project$Main$multipleChoiceQuestionTemplate = function (question) {
	var answerLst = A2(
		$elm$core$List$indexedMap,
		F2(
			function (i, a) {
				return A2(
					$author$project$Main$multipleChoiceAnswerTemplate,
					a,
					_Utils_eq(i, question.correct - 1));
			}),
		question.answers);
	var answersStr = A2($elm$core$String$join, '', answerLst);
	return '\n<question type="multichoice">\n     <name>\n         <text>' + (question.question + ('</text>\n     </name>\n     <questiontext format="html">\n         <text>' + (question.question + ('</text>\n     </questiontext>\n' + (answersStr + '\n </question>\n')))));
};
var $author$project$Main$quizTemplate = '<?xml version="1.0" ?>\n<quiz>\n{{questions}}\n</quiz>\n';
...
var $author$project$Main$buildMoodleXml = function (questions) {
	return A3(
		...
		A2(
			$elm$core$String$join,
			'',
			A2($elm$core$List$map, $author$project$Main$multipleChoiceQuestionTemplate, questions)),
		$author$project$Main$quizTemplate);
};
var $author$project$Main$Question = F3(
	function (question, correct, answers) {
		return {answers: answers, correct: correct, question: question};
	});
var $author$project$Main$parseSingleCsvLine = F2(
	function (sep, line) {
		var fields = A2($elm$core$String$split, sep, line);
		var result = function () {
			if (fields.b && fields.b.b) {
				var question = fields.a;
				var _v1 = fields.b;
				var correct = _v1.a;
				var answers = _v1.b;
				var _v2 = $elm$core$String$toInt(correct);
				if (_v2.$ === 'Nothing') {
					return $elm$core$Result$Err('could not convert correct answer to a number in this line [' + (line + ']'));
				} else {
					var correctInt = _v2.a;
					return $elm$core$Result$Ok(
						A3($author$project$Main$Question, question, correctInt, answers));
				}
			} else {
				return $elm$core$Result$Err('missing fields in this line [' + (line + ']'));
			}
		}();
		return result;
	});
...
var $author$project$Main$parseCsvMultipleChoiceFile = F3(
	function (sep, hasHeader, text) {
		var lines = A2($elm$core$String$split, '\n', text);
		var content = function () {
			if (hasHeader) {
				var _v2 = $elm$core$List$tail(lines);
				if (_v2.$ === 'Nothing') {
					return _List_Nil;
				} else {
					var l = _v2.a;
		...
		var resultQuestions = A2(
			$elm$core$List$map,
			function (l) {
				return A2($author$project$Main$parseSingleCsvLine, sep, l);
			},
			content);
		var appendQuestionError = F2(
			function (resultQ, _v0) {
				var qList = _v0.a;
				var eList = _v0.b;
				if (resultQ.$ === 'Err') {
					var error = resultQ.a;
					return _Utils_Tuple2(
						qList,
						A2($elm$core$List$cons, error, eList));
				} else {
					var q = resultQ.a;
					return _Utils_Tuple2(
						A2($elm$core$List$cons, q, qList),
						eList);
				}
			});
		var res = A3(
...
var $author$project$Main$CsvLoaded = F2(
	function (a, b) {
		return {$: 'CsvLoaded', a: a, b: b};
	});
...
var $author$project$Main$readFile = function (file) {
	return A2(
		$elm$core$Task$perform,
		$author$project$Main$CsvLoaded(file),
		$elm$file$File$toString(file));
};
var $author$project$Main$GotFiles = F2(
	function (a, b) {
...
var $author$project$Main$requestFiles = A2(
	$elm$file$File$Select$files,
	_List_fromArray(
		['text']),
	$author$project$Main$GotFiles);
...
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'RequestFiles':
				return _Utils_Tuple2(model, $author$project$Main$requestFiles);
			case 'GotFiles':
				var file = msg.a;
				var otherFiles = msg.b;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							files: A2($elm$core$List$cons, file, otherFiles),
							log: _Utils_ap(
								model.log,
								_List_fromArray(
									['loaded files']))
					...
					_Utils_update(
						model,
						{
							log: _Utils_ap(
								model.log,
								_List_fromArray(
									['changed sep to ' + newSep])),
							sep: newSep
						}),
					$elm$core$Platform$Cmd$none);
			case 'ClickedConvert':
				var cmd = $elm$core$Platform$Cmd$batch(
					A2($elm$core$List$map, $author$project$Main$readFile, model.files));
				return _Utils_Tuple2(model, cmd);
			case 'ClickedDownload':
				var downloadCmds = $elm$core$Platform$Cmd$batch(
					A2(
						$elm$core$List$indexedMap,
						F2(
							function (i, c) {
								return A3(
									...
									'questions' + ($elm$core$String$fromInt(i) + '.xml'),
									'text/xml',
									c.result);
							}),
						model.conversions));
				return _Utils_Tuple2(model, downloadCmds);
			case 'CsvLoaded':
				var file = msg.a;
				var content = msg.b;
				var _v1 = A3($author$project$Main$parseCsvMultipleChoiceFile, model.sep, true, content);
				var questions = _v1.a;
				var errors = _v1.b;
				var moodleXml = $author$project$Main$buildMoodleXml(questions);
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							conversions: _Utils_ap(
								model.conversions,
								_List_fromArray(
									[
										A4($author$project$Main$Conversion, file, questions, errors, moodleXml)
									])),
							log: _Utils_ap(
								model.log,
								_Utils_ap(
									...
									_List_fromArray(
										[
											'\nerrors:' + A2($elm$core$String$join, '\n', errors)
										])))
						}),
					$elm$core$Platform$Cmd$none);
			case 'ClickedDownloadOpenOfficeTemplate':
				var downloadCmd = $elm$file$File$Download$url('public/template/template.ods');
				return _Utils_Tuple2(model, downloadCmd);
			default:
				var downloadCmd = $elm$file$File$Download$url('public/template/template.xlsx');
				return _Utils_Tuple2(model, downloadCmd);
		}
	});
var $author$project$Main$ClickedConvert = {$: 'ClickedConvert'};
var $author$project$Main$ClickedDownload = {$: 'ClickedDownload'};
...
var $author$project$Main$blue = A3($mdgriffith$elm_ui$Element$rgb255, 238, 238, 238);
...
var $author$project$Main$ChangedSeperator = function (a) {
	return {$: 'ChangedSeperator', a: a};
};
...
var $author$project$Main$chooseSep = function (model) {
	return A2(
			...
			]),
		{
			label: A2(
				...
				_List_Nil,
				$mdgriffith$elm_ui$Element$text('Choose seperator between fields:')),
			onChange: $author$project$Main$ChangedSeperator,
			options: _List_fromArray(
				[
					...
					$mdgriffith$elm_ui$Element$Input$option,
					'custom',
					$mdgriffith$elm_ui$Element$text('custom'))
				]),
			selected: $elm$core$Maybe$Just(model.sep)
		});
};
...
var $author$project$Main$RequestFiles = {$: 'RequestFiles'};
...
var $author$project$Main$purple = A3($mdgriffith$elm_ui$Element$rgb255, 238, 238, 150);
var $author$project$Main$uploadFilesButton = A2(
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$mdgriffith$elm_ui$Element$Background$color($author$project$Main$purple)
				])),
			$mdgriffith$elm_ui$Element$centerX,
			$mdgriffith$elm_ui$Element$padding(10)
		]),
	{
		label: $mdgriffith$elm_ui$Element$text('Upload files'),
		onPress: $elm$core$Maybe$Just($author$project$Main$RequestFiles)
	});
var $author$project$Main$body = function (model) {
	return A2(
		...
		A2(
			...
			_List_fromArray(
				[
					$author$project$Main$uploadFilesButton,
					$author$project$Main$chooseSep(model),
					A2(
					$mdgriffith$elm_ui$Element$row,
					_List_fromArray(
						[
					...
					_List_fromArray(
						[
								...
								]),
							{
								...
								onPress: $elm$core$Maybe$Just($author$project$Main$ClickedConvert)
							}),
							A2(
							$mdgriffith$elm_ui$Element$Input$button,
							_List_fromArray(
								...
								]),
							{
								...
								onPress: $elm$core$Maybe$Just($author$project$Main$ClickedDownload)
							})
						]))
				]),
			A2(
				$elm$core$List$map,
				function (info) {
					return $mdgriffith$elm_ui$Element$text(info);
				},
				model.log)));
};
var $author$project$Main$ClickedDownloadExcelTemplate = {$: 'ClickedDownloadExcelTemplate'};
var $author$project$Main$ClickedDownloadOpenOfficeTemplate = {$: 'ClickedDownloadOpenOfficeTemplate'};
var $author$project$Main$footer = A2(
	...
	_List_fromArray(
		[
			$mdgriffith$elm_ui$Element$text('Download spreadsheet templates'),
			A2(
			$mdgriffith$elm_ui$Element$row,
			_List_fromArray(
				[
			...
			_List_fromArray(
				[
					...
					{
						label: $mdgriffith$elm_ui$Element$text('OpenOffice'),
						onPress: $elm$core$Maybe$Just($author$project$Main$ClickedDownloadOpenOfficeTemplate)
					}),
					A2(
					$mdgriffith$elm_ui$Element$Input$button,
					_List_fromArray(
					...
					{
						label: $mdgriffith$elm_ui$Element$text('MS Excel'),
						onPress: $elm$core$Maybe$Just($author$project$Main$ClickedDownloadExcelTemplate)
					})
				])),
			A2(
			$mdgriffith$elm_ui$Element$el,
			_List_fromArray(
				[$mdgriffith$elm_ui$Element$centerX]),
			$mdgriffith$elm_ui$Element$text('Diogo Silva'))
		]));
...
var $author$project$Main$header = A2(
	...
	_List_fromArray(
		[
			...
			$mdgriffith$elm_ui$Element$text('Easy Moodle Questions'))
		]));
